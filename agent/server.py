...
async def monitor_log_file():
    """
    背景任務：每5秒掃描一次 log.json，偵測新的一筆 observation
    """
    ...
    while True:
        ...
        try:
            ...
            if last_processed_index < len(logs):
                ...
                # 儲存 output
                store_agent_response(result, f"{OUTPUT_DIR}/{timestamp}_output.json")
...
async def add_daily_plan(item: dict):
    # 直接寫入到 daily_plan
    plan_manager.plan['daily_plan'].append(item)
    plan_manager.save()
    return {"status": "daily plan 新增成功"}
...
async def add_current_plan(item: dict):
    plan_manager.add_to_current_plan(item['time'], item['action'])
    return {"status": "current plan 新增成功"}
